// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from radar_msg:msg/Ptu.idl
// generated code does not contain a copyright notice
#include "radar_msg/msg/detail/ptu__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `header`
#include "std_msgs/msg/detail/header__functions.h"

bool
radar_msg__msg__Ptu__init(radar_msg__msg__Ptu * msg)
{
  if (!msg) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__init(&msg->header)) {
    radar_msg__msg__Ptu__fini(msg);
    return false;
  }
  // pan_deg
  // tilt_deg
  return true;
}

void
radar_msg__msg__Ptu__fini(radar_msg__msg__Ptu * msg)
{
  if (!msg) {
    return;
  }
  // header
  std_msgs__msg__Header__fini(&msg->header);
  // pan_deg
  // tilt_deg
}

bool
radar_msg__msg__Ptu__are_equal(const radar_msg__msg__Ptu * lhs, const radar_msg__msg__Ptu * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__are_equal(
      &(lhs->header), &(rhs->header)))
  {
    return false;
  }
  // pan_deg
  if (lhs->pan_deg != rhs->pan_deg) {
    return false;
  }
  // tilt_deg
  if (lhs->tilt_deg != rhs->tilt_deg) {
    return false;
  }
  return true;
}

bool
radar_msg__msg__Ptu__copy(
  const radar_msg__msg__Ptu * input,
  radar_msg__msg__Ptu * output)
{
  if (!input || !output) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__copy(
      &(input->header), &(output->header)))
  {
    return false;
  }
  // pan_deg
  output->pan_deg = input->pan_deg;
  // tilt_deg
  output->tilt_deg = input->tilt_deg;
  return true;
}

radar_msg__msg__Ptu *
radar_msg__msg__Ptu__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  radar_msg__msg__Ptu * msg = (radar_msg__msg__Ptu *)allocator.allocate(sizeof(radar_msg__msg__Ptu), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(radar_msg__msg__Ptu));
  bool success = radar_msg__msg__Ptu__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
radar_msg__msg__Ptu__destroy(radar_msg__msg__Ptu * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    radar_msg__msg__Ptu__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
radar_msg__msg__Ptu__Sequence__init(radar_msg__msg__Ptu__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  radar_msg__msg__Ptu * data = NULL;

  if (size) {
    data = (radar_msg__msg__Ptu *)allocator.zero_allocate(size, sizeof(radar_msg__msg__Ptu), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = radar_msg__msg__Ptu__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        radar_msg__msg__Ptu__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
radar_msg__msg__Ptu__Sequence__fini(radar_msg__msg__Ptu__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      radar_msg__msg__Ptu__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

radar_msg__msg__Ptu__Sequence *
radar_msg__msg__Ptu__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  radar_msg__msg__Ptu__Sequence * array = (radar_msg__msg__Ptu__Sequence *)allocator.allocate(sizeof(radar_msg__msg__Ptu__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = radar_msg__msg__Ptu__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
radar_msg__msg__Ptu__Sequence__destroy(radar_msg__msg__Ptu__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    radar_msg__msg__Ptu__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
radar_msg__msg__Ptu__Sequence__are_equal(const radar_msg__msg__Ptu__Sequence * lhs, const radar_msg__msg__Ptu__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!radar_msg__msg__Ptu__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
radar_msg__msg__Ptu__Sequence__copy(
  const radar_msg__msg__Ptu__Sequence * input,
  radar_msg__msg__Ptu__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(radar_msg__msg__Ptu);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    radar_msg__msg__Ptu * data =
      (radar_msg__msg__Ptu *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!radar_msg__msg__Ptu__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          radar_msg__msg__Ptu__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!radar_msg__msg__Ptu__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
