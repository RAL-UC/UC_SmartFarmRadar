...
#from radar_package.parametros import * # carga de parametros
from matplotlib.widgets import Slider, RadioButtons # interfaz de usuario
...
class RadarVisualizer(Node):
    def __init__(self):
        ...
        self.filtered_data = None # data amplitud filtrada "eje y" desplazada en offset (dB)
    ...
    def update_display(self, angle_val: int):
        ...
        if m == "false_alarm":
            thresh, targets, noise_variance = cfar_adaptive_edge(mag, num_guard_cells=ng, num_ref_cells=nr, bias=b, cfar_method=m, fa_rate=fa_rate)
            ...
            thresh, targets = cfar_adaptive_edge(mag, num_guard_cells=ng, num_ref_cells=nr, bias=b, cfar_method=m)
            #thresh = self.unpad(thresh, total_ext)
            #targets = np.ma.array(self.unpad(targets, total_ext), mask=self.unpad(targets.mask, total_ext))
            self.line_noise.set_visible(False)
    ...
    def listener_callback(self, msg: RadarData):
        ...
        has_data = self.filtered_data is not None
        use_fa = (self.radio_method.value_selected == "false_alarm")
